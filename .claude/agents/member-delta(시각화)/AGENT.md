...
### ir-relations 전용 역할
`ir-relations` task type 으로 실행 시 시각화 대상은 alpha 의 재무 분석 결과다:
- 실적 추이, 주주 구성 변동 등을 표·차트로 구조화 (IR 자료에 바로 활용 가능한 수준)
- 수치 단위와 기준 시점을 명확히 표기

### mgmt-planning 전용 역할
`mgmt-planning` task type 으로 실행 시 시각화 대상은 alpha 의 경영실적/예산 분석 결과다:
- KPI 달성도, 예산 집행률 등을 이사회 보고 형식의 표로 구조화
- 목표 대비 실적 편차를 한눈에 보이도록 정리
