...
**사이클**: 2 / 3 (사용자 피드백에 따른 범위 재조정 REASSIGN 반영)
...
**각주(테이블 ①·② 정합성)**: 테이블 ①의 2022년 바로고 처리건수 "1억 9,700만 건"은 자사 확정
연간치이고, 테이블 ②의 "약 1,800만 건"은 3사 비교를 위한 **업계추산 월평균**치로 산정 기준이
다르다. 전자를 단순 월평균하면 약 1,642만 건(1억 9,700만 ÷ 12)으로, 후자(1,800만 건)와 약
9~10% 차이가 난다 — 이는 원문에도 설명이 없는 차이이며, "업계추산"이라는 표기 자체가 이미
비공식 추정치임을 시사한다. 두 수치를 동일 지표로 오인하지 말 것.
